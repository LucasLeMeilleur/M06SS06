...
{
    return this->Envoyer(requete.c_str(), requete.length());
}
...
int IRClientTCP::Recevoir(std::string & reponse)
{
	char  rep[1500];
	int nbOctets = Recevoir(rep, 1500);
	reponse = std::string(rep,nbOctets);
	return nbOctets;
}
