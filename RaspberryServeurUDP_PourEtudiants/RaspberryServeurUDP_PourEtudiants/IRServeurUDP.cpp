...
int IRServeurUDP::RecevoirDesOctets(unsigned char * tableau, int tailleMax)
{
    #if defined(_WIN32) || defined(_WIN64)
    int length = sizeof(from); // Doit être initialisé ???
    #else
    ...
    // BUG corrigé : la structure from doit être initialisée à 0 (voir les spécifications de recfrom)
    memset(&from, 0, sizeof(from));
    unsigned int nbCaracteresRecus = recvfrom(m_socketServeurUDP, (char *)tableau, tailleMax, 0, (struct sockaddr*)&from, &length);
